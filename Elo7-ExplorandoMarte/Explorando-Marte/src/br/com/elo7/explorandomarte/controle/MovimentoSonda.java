...
import br.com.elo7.explorandomarte.enums.Instrucao;
 ...
 * @since 31/10/2015
 *
 * Responsável por executar as instruções de cada sonda informada, validando regras funcionais.
 */
public class MovimentoSonda {
	...
	public Sonda executarInstrucao(Sonda sonda) throws CoordenadaForaDoEixoDoPlanaltoException, CamposInvalidosException {
		...
		for (int i = 0; i < sonda.getInstrucaoMovimentos().length(); i++) {
			...
			if (sonda.getMovimentoAtual() != Instrucao.MOVE.getValor()) {
...
//		return this.cancelarInstrucao;
	...
	public void validarMovimentoAtual(int i, Sonda sonda) throws CamposInvalidosException {
		...
		if ((sonda.getMovimentoAtual() != Instrucao.MOVE.getValor()) 
				&& (sonda.getMovimentoAtual() != Instrucao.LEFT.getValor()) 
					&& (sonda.getMovimentoAtual() != Instrucao.RIGHT.getValor())) {
